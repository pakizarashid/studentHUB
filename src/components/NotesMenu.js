...
export default function NotesMenu({ userData }) {
  ...
  console.log("Updated user data:", updatedUserData); // Log the updated user data
    ...
    for (const semesterKey in semesters) {
      ...
      for (const courseKey in semesterCourses) {
        const course = semesterCourses[courseKey];
         ...
         console.log(`Course data for key ${courseKey}:`, course);
...
const styles = StyleSheet.create({
  ...
  FAB: {
    ...
    right: "5%",
    bottom: "5%",
    backgroundColor: "rgba(10,64,129,0.35)",
  },
});
